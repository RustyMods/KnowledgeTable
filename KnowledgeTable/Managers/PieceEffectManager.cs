...
namespace KnowledgeTable.Managers;
...
public static class PieceEffectManager
{
    ...
    private static class FindAndAddPieceEffects
    {
        private static void Postfix(ZNetScene __instance)
        {
            if (!__instance) return;
            AddPieceEffects(__instance);
        }
    }
    private static void AddPieceEffects(ZNetScene instance)
    {
        if (!instance) return;
        foreach (GameObject prefab in PrefabsToSet)
        {
            ...
            if (!Object) continue;
            SetPieceScript(instance, Object, "vfx_Place_workbench", "sfx_build_hammer_wood");
            SetWearNTearScript(instance, Object,
                "vfx_SawDust", "sfx_wood_destroyed",
                "vfx_SawDust", "sfx_rock_hit",
                "vfx_Place_workbench", 0f);
        }
    }
